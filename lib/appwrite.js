...
export async function createUser(email, password, username) {
  try {
    ...
    // O newUser retornado aqui já é o documento de perfil criado
    const newUser = await databases.createDocument(
      ...
      newAccount.$id, // Usa o ID da conta como ID do documento de perfil
      {
        ...
        username: username,
        avatar: avatarUrl,
        // Ao criar o usuário, você pode definir o role padrão aqui
        // role: 'enfermeiro', // Por exemplo, se enfermeiro for o padrão
      }
    );
...
export async function signIn(email, password) {
  try {
    // Tenta obter a sessão atual para verificar se já está logado
    // Se a sessão existe e é válida, delete-a para garantir um novo login limpo
    try {
      ...
      if (sessionError.code !== 401) { // 401 é "User (role:guests) missing scope(account)", esperado para não logado
...
export const getUserPosts = async (userId) => {
  try {
    ...
    const postsWithCreator = await Promise.all(posts.documents.map(async (post) => {
      if (post.uploaderId) {
        try {
          // Busca o perfil do uploader pelo ID do documento do usuário
          const uploaderProfile = await databases.listDocuments(
            databaseId,
            userCollectionId,
            [Query.equal('$id', post.uploaderId)] // Correto: busca pelo ID do documento do usuário
          );
          if (uploaderProfile.documents.length > 0) {
            post.creator = {
              ...
              role: uploaderProfile.documents[0].role // Inclui o role do criador
            };
          } else {
...
// ATENÇÃO: Esta função foi corrigida para buscar pelo ID do documento do perfil
export const getUserProfile = async (userId) => { // Renomeado 'accountId' para 'userId' para maior clareza
  try {
    const users = await databases.listDocuments(
      databaseId,
      userCollectionId,
      [Query.equal('$id', userId)] // CORRIGIDO: Agora busca pelo ID do documento do perfil
    );
    // console.log('getUserProfile result:', users.documents[0]); // Debug
    return users.documents[0]; // Retorna o primeiro documento encontrado
  } catch (error) {
...
export async function ensureUserProfile(user) {
  try {
    // Busca o perfil pelo $id do usuário Appwrite (que é o ID do documento de perfil)
    ...
    const newProfile = await databases.createDocument(
      ...
      user.$id, // O $id do documento será igual ao $id do usuário Appwrite!
      {
        ...
        avatar: avatarUrl,
        // Role padrão para novos usuários (você pode ajustar isso)
        role: 'enfermeiro', 
      }
    );
    return newProfile;
