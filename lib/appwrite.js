...
export const getUserPosts = async (userId) => {
  try {
    ...
    const postsWithCreatorAndDoctor = await Promise.all(posts.documents.map(async (post) => {
      // Fetch Creator (Uploader) Profile
      if (post.uploaderId) {
        try {
      ...
      // Fetch Laudation Doctor Profile if ECG is lauded and doctor ID exists
      if (post.status === 'lauded' && post.laudationDoctorId) {
        try {
          ...
          if (doctorProfile.documents.length > 0) {
            ...
            console.warn(`Perfil do médico com ID ${post.laudationDoctorId} não encontrado.`);
            post.laudationDoctor = { username: 'Unknown Doctor', avatar: images.profile };
          }
        } catch (doctorProfileError) {
          console.error(`Erro ao buscar perfil do médico ${post.laudationDoctorId}:`, doctorProfileError);
